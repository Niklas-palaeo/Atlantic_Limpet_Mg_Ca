...
There are two main locations that we sourced specimens from: the island of West Ray in Orkney (UK) and the Beagle Channel in Tierra del Fuego (Argentina). A modern shell on West Ray was collected in Rack Wick Bay in August 2009, which experiences virtually no freshwater input. The temperature
...
```{r SST Data}
#| fig-cap: SST values averaged over the last 20 years. SST values were sourced from [@europeanunion-copernicusmarineservice2015]
#| fig-height: 10
#| fig-width: 10

# Read the CSV files, skipping the comments
df1 <- read_csv(here("data","Westray-SST.csv"), comment = "#")
df2 <- read_csv(here("data","Tierra_del_Fuego-SST.csv"), comment = "#")

# Combine the datasets with new names and add year and month columns
combined_df <- bind_rows(df1 %>% mutate(dataset = "Westray"),
                         df2 %>% mutate(dataset = "Tierra del Fuego")) %>%
